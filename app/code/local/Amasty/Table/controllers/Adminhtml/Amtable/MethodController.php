    ...
    public function saveAction()
    {
        ...
        if ($data) {
            if (($data['min_rate'] > $data['max_rate']) && ($data['max_rate'] > 0) && ($data['min_rate'] > 0)) {
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('amtable')->__('Minimal rate must be less than maximal rate, please check your restrictions'));
                $this->_redirect('*/*/edit', array('id' => $model->getId()));
                return;
            }

            try {
                $data['comment_img'] = $model->saveImage($id, $data);
                $model->setData($data);
                $model->setId($id);
