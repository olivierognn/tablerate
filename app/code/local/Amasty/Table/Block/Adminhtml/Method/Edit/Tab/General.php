...
class Amasty_Table_Block_Adminhtml_Method_Edit_Tab_General extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        $fldInfo->addField('comment', 'textarea', array(
            ...
            'name'      => 'comment',
            'note'      => $hlp->escapeHtml(__('HTML tags <b>, <u>, <i>, <s> and inline CSS are supported.
                    For example: This is a <b style="color:blue;">Blue text</b>. To learn more, refer to this page: '))
                . '<a href="' . $hlp->escapeUrl('https://www.w3schools.com/html/html_css.asp') . '" title="' . $hlp->__('HTML Styles - CSS')
                . '" target="_blank">' . $hlp->escapeUrl('https://www.w3schools.com/html/html_css.asp') . '</a>',
        ));

        $fldInfo->addField('comment_img', 'image', array(
            'label'     => $this->__('Image'),
            'name'      => 'comment_img',
            'note'      => $hlp->__('Please use {IMG} in Comment field to insert an image'),
        ));
