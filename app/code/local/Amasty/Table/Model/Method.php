...
class Amasty_Table_Model_Method extends Mage_Core_Model_Abstract
{
    ...
    public function addComment($html, $storeId)
    {
        ...
        if (!empty($matches[0])) {
            ...
            foreach ($matches[0] as $key => $value) {
                ...
                $imageUrl = $this->getImageUrl($methodId);
                $imageHtml = $imageUrl ? '</div><img src="' . $imageUrl . '" /><div>' : $imageUrl;
                ...
                $comment = $hashMethods[$methodId] === ''
                    ? $hashMethods[$methodId]
                    : '<div>' . $hashMethods[$methodId] . '</div>';

                $to[] = $matches[0][$key] . $comment;
            }
    ...
    {
        if ($this->getId() != $methodId) {
            $this->load($methodId);
        }
        $imageName = $this->getCommentImg();
        if ($imageName) {
            return Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . DS . $imageName;
        }
