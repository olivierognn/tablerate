...
class Amasty_Table_Model_Rate extends Mage_Core_Model_Abstract
{
     ...
     * @param Amasty_Table_Model_Mysql4_Method_Collection $collection
     *
     * @return array
     */
    public function findBy($request, $collection)
    {
        ...
        /** @var Amasty_Table_Model_Mysql4_Rate_Collection $allRates */
        $allRates = $this->getResourceCollection();
        $ratesData = $allRates->getRatesWithFilters($methodIds);
        $ratesTypes = array();
        ...
        foreach ($ratesTypes as $key => $value) {
            ...
            foreach ($intersectTypes[$key] as $shippingType) {
                ...
                if ($allTotals['qty'] > 0
                    && (!Mage::getStoreConfig('carriers/amtable/dont_split')
                    ...
                    $ratesData = $allRates->getRatesWithFilters(
                        $methodIds,
                        false,
                        array($request, $totals, $shippingType)
                    );
    ...
    {
        $shippingType = $this->getProductShippingType($item->getProduct());
        $this->_existingShippingTypes[] = $shippingType;
    }
     ...
     * @return int
     */
    protected function getProductShippingType($product)
    {
        $shippingType = $product->getData('am_shipping_type');
        if ($shippingType === null) {
            $shippingType = (int)Mage::getResourceModel('catalog/product')
                ->getAttributeRawValue($product->getEntityId(), 'am_shipping_type', $product->getStoreId());
        ...
        foreach ($request->getAllItems() as $item) {
            ...
            if (($item->getHasChildren() && $typeId == 'configurable' && $configurableSetting == '0')
                ...
                foreach ($item->getChildren() as $child) {
                    ...
                    if (($this->getProductShippingType($item->getProduct()) != $shippingType) && ($shippingType != 0)) {
                        continue;
                    }
    ...
    public function import($methodId, $fileName)
    {
        ...
        while (($line = fgetcsv($fp, self::MAX_LINE_LENGTH, ',', '"')) !== false) {
            ...
            for ($i = 0; $i < self::COL_NUMS - self::HIDDEN_COLUMNS; $i++) {
                ...
                if ($i != self::COL_NUMS - self::HIDDEN_COLUMNS - 1
                    && preg_match(
                        '/^([0-9]+?\,)+?[0-9]+?\.[0-9]+$/',
                        $line[$i]
                    )
    ...
    public function returnErrors($data, $methodId, $currLineNum, $err)
    {
        ...
        if ($errText) {
            ...
            foreach ($newData AS $key => $strNewData) {
                $newData[$key] = Mage::helper('amtable')->unserialize($strNewData);
            }
