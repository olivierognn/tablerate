...
class Amasty_Table_Model_Observer
{
    ...
    public function addMethodComment(Varien_Event_Observer $observer)
    {
        ...
        if (Mage::getConfig()->getModuleConfig('Inovarti_Onestepcheckout')->is('active', 'true')){
            $firstBlock = Mage::getConfig()->getBlockClassName('onestepcheckout/onestep_form_shippingmethod');
        }
        $secondBlock = Mage::getConfig()->getBlockClassName('checkout/cart_shipping');
        if (Mage::getConfig()->getModuleConfig('Ced_CsMultiShipping')->is('active', 'true')){
            $secondBlock = Mage::getConfig()->getBlockClassName('csmultishipping/cart_shipping');
        }
