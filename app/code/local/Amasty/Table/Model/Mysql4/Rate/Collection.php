...
class Amasty_Table_Model_Mysql4_Rate_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    public function getRatesWithFilters($methodIds, $resetColumns = false, $params = null)
    {
        ...
        if ($resetColumns) {
            ...
            $this->getSelect()->columns(array('method_id', 'shipping_type'));
        }

        if ($params) {
            list($request, $totals, $shippingType) = $params;
            $this->addAddressFilters($request);
            $this->addTotalsFilters($totals, $shippingType);
        }
     ...
     * @return $this
     */
    protected function addMethodFilters($methodIds)
    {
